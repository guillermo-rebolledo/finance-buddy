...
async function exportRequest(page: Page, data: Record<string, unknown>) {
  return page.request.post("/api/export", {
    headers: { Origin: origin },
    data: { kind: "week", date: "2026-09-11", ...data },
  });
}
function category(
...
test("export requires the owner's own session and a same-origin JSON request", async ({
  ...
  expect(
    (
      await request.post("/api/export", {
        headers: { Origin: origin },
        data: { id: randomUUID(), kind: "week", date: "2026-09-11" },
      })
    ).status(),
  ).toBe(401);
  expect(await createdSpreadsheets()).toHaveLength(0);
  await overview(page);
  await connectExport(page);
  const foreign = await page.request.post("/api/export", {
    headers: { Origin: "https://attacker.example" },
    data: { id: randomUUID(), kind: "week", date: "2026-09-11" },
  });
  expect(foreign.status()).toBe(403);
  const form = await page.request.post("/api/export", {
    headers: { Origin: origin, "Content-Type": "text/plain" },
    data: JSON.stringify({ id: randomUUID(), kind: "week" }),
  });
...
async function connectExport(page: Page) {
  ...
  await connectSheets(page);
  await expect(
    page.getByText("Google Sheets export is connected"),
  ).toBeVisible();
}
