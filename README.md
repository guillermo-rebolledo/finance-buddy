...
Both writes refuse a period that has already ended with `period_ended`, before changing anything; the current period still accepts changes on its last day. Each replies with the budget view that now applies to the named period, or `null`, read after the change commits, and repeating the identical request leaves the same budgets and the same reply. Rows store spans of periods rather than one row per period, and a period's budget is resolved when it is read, so nothing runs on a schedule.
