...
| `period_ended`           | 409    | The named period has already ended, so its budget is read-only.                                  |
...
5. **Act on codes, not messages.** `unauthenticated`: delete the token and sign in again. `upgrade_required`: ask for an update and send nothing more. `reconnect_required`: tell the owner to connect Google Sheets on the web dashboard. `invalid_field`: mark `field`. `period_ended`: the day, week, or month is history, so show its budget as read-only and do not retry. `unavailable`: retry later.
...
| `PATCH /api/journal`                       | the same body; `id` names the entry                                                                            | `{ saved: true }`, plus `budget` for an expense or refund whose date has a budget                                                                           | `invalid_field`, `not_confirmed`                                                                             |
...
| `GET /api/budgets?before=`                | optional `before` cursor for more of `past`                                                                    | `{ today, currency, now: { day, week, month }, repeating, upcoming, past, nextBefore }`                                                                     | `invalid_field`                                                                                              |
| `PUT /api/budgets?kind=&date=`            | `{ amount, oneOff }`                                                                                           | `{ saved: true, budget }`                                                                                                                                    | `invalid_period`, `invalid_field`, `period_ended`, `not_confirmed`                                           |
| `DELETE /api/budgets?kind=&date=`         | `{ scope: "period" \| "onward" }`                                                                              | `{ saved: true, budget }`                                                                                                                                    | `invalid_period`, `invalid_field`, `period_ended`, `not_confirmed`                                           |
...
`PUT /api/budgets?kind=&date=` names the period exactly as the summary does and takes `{ amount, oneOff }`. `amount` follows the Amount format but may be zero, and `oneOff` must be a boolean.

With `oneOff: true` it sets or replaces the one-off budget of the resolved period, current or future, and the budget view reports `repeats: false`. A one-off budget takes precedence over the repeating budget for its own period only, so the repeating budget applies again from the next period, and a period never has more than one budget of its kind. `DELETE` with `{ scope: "period" }` removes it, so the repeating budget, if any, applies to that period again; removing one that does not exist succeeds without changing anything.

With `oneOff: false` the budget repeats from the resolved period through every later period of that kind. Setting it again from the period where it starts replaces the amount from there on. Setting it from a later period, including the current one or a future one, takes over from that period: the earlier budget ends at the period before, ended periods keep the amount that applied to them, and the new amount runs only until the next scheduled change, which survives. With MXN 2,000 from September and MXN 8,000 from October, changing September onward to MXN 3,000 leaves October's MXN 8,000 in place. Setting the repeating amount from a period that has a one-off budget removes that one-off budget, which is how unticking One-off makes its amount the repeating amount from that period on; one-off budgets on other periods are untouched.

`DELETE /api/budgets?kind=&date=` with `{ scope: "onward" }` stops repeating from the resolved period: the repeating budget that contains it ends at the period before, or goes if it starts there, and every scheduled change after it goes too. Ended periods keep their budgets, one-off budgets stay as they are, and stopping when nothing repeats succeeds without changing anything. `scope` is refused on `scope` when it is anything else.

Both writes refuse a period that has already ended with `period_ended`, before changing anything; the current period still accepts changes on its last day. Changes to one person's budgets of one kind run one at a time, and a change is judged when its turn comes, so one that waited past midnight is refused if its period ended meanwhile. Each replies with the budget view that now applies to the named period, or `null`, read after the change commits, and repeating the identical request leaves the same budgets and the same reply. Rows store spans of periods rather than one row per period, and a period's budget is resolved when it is read, so nothing runs on a schedule.

The remaining budget is the budget minus total expenses. It is never clamped, so it can exceed the budget when refunds outweigh expenses. A period is over budget only when its total expenses exceed the budget, which the dashboard reads as "Over by MXN X"; spending exactly the budget leaves MXN 0.00. The dashboard shows a read-only budget card for the period on screen, labelled Repeating or One-off, or says the period has no budget, and links to Budgets. For an ended period the card says whether it ended under or over budget, as "Under by MXN X" or "Over by MXN X". Nothing carries over between periods.
...
Recording or correcting an expense or refund replies `{ saved: true, budget }`, where `budget` is the budget view, read after the write commits, for the shortest period (day, then week, then month) that contains the saved entry's movement date and has a budget. A past-dated entry therefore reports its own period, a correction reports the entry as corrected, and a refund's reply shows the room it gave back. `budget` is left out for income, when no period containing the date has a budget, and when the budget cannot be read after a save that did complete. The registry's confirmation adds it as a line such as "Entry saved. MXN 1,240.00 left this week." or "Entry saved. Over by MXN 300.00 for the week of 31 Aug – 6 Sep."

The Budgets page, between Dashboard and Categories in the navigation, lists today's day, week, and month under Now, each with its figures, a Repeating or One-off label, and a Change action, or a Set budget action, and explains budgets to a person who has none yet. Its form takes a period kind, any date in the period (the current one by default), an amount, and a One-off (this period only) checkbox, unticked by default. It names the resolved period, such as "Week of 14–20 Sep", fills in the budget that already applies to it, ticks One-off when that budget is a one-off budget and offers to remove it, and says when the chosen period has ended. Under Repeating, each repeating budget and each scheduled change is a row with its amount and the period it starts from. Change opens the form on that row's first period still to come, and Stop asks for confirmation, explaining that scheduled changes go too, before stopping from that period. Upcoming one-offs lists the one-off budgets of future periods with Change and Remove. Past lists ended periods that had a budget, newest first and read-only, each marked "Under by MXN X" or "Over by MXN X", 20 at a time with Show more.
...
Reports use locally vendored [pdfcn](https://www.pdfcn.dev/) components and Forme 0.23.0 for server-side rendering. The template is `src/components/pdf/financial-report.tsx`; colors, typography, and spacing live in `src/lib/pdf-themes/report.ts`. The print-first layout has a green masthead, serif headings, a shaded totals card, striped financial-movement rows, and repeating page numbers. Rendering needs no API key, browser process, or network font requests.

The `@pdfcn` registry is configured in `components.json`. Components under `src/components/pdf/` were installed from its `forme/` namespace and adapted for this app: native Forme tables handle repeated headers and oversized rows, headings use semantic heading primitives, and the theme is immutable across requests. Preserve these adaptations when updating registry source. `next.config.ts` externalizes Forme so its packaged WASM is available in the server build.
