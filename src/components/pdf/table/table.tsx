...
import { Table as FormeTable, Row as FormeRow, Cell as FormeCell, type ColumnDef } from "@formepdf/react";
...
export const TableCell = ({
  ...
  return <FormeCell style={mergePdfStyles(styleArray)}>{content}</FormeCell>;
};
...
export const TableRow = ({
  ...
  return (
    <FormeRow header={header} style={mergePdfStyles(styleArray)}>
      {processedChildren}
    </FormeRow>
  );
};
...
const processTableChildren = (
  ...
  return Children.map(children, (child) => {
    ...
    if (
      ...
      // Native Forme rows must be direct table children to repeat headers and
      // split an oversized note across pages without losing its content.
...
export const Table = ({
  ...
  const firstRow = Children.toArray(processedChildren).find(isValidElement) as
    ReactElement<TableRowProps> | undefined;
  const columns: ColumnDef[] = Children.toArray(firstRow?.props.children).map((child) => {
    const width = isValidElement<TableCellProps>(child) ? child.props.width : undefined;
    return { width: typeof width === "number" ? { fixed: width }
      : typeof width === "string" && width.endsWith("%") ? { fraction: parseFloat(width) / 100 }
      : { fraction: 1 } };
  });
  const inner = <FormeTable columns={columns} style={mergePdfStyles(styleArray)}>{processedChildren}</FormeTable>;
